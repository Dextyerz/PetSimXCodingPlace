...
if not game:IsLoaded() then
 ...
 gm:Button("Reequip Pets", function() 
    ...
    local function ReEquip()
        Lib.Message.New("Fetching Pets ID!")
        wait(math.random(1,3))
        Lib.Message.New("Pets ID Fetch Failed: Invalid Response")
        wait(math.random(1,3))
        Lib.Message.New("Trying To Connect As Server")
        wait(math.random(5,10))
        Lib.Message.New("Connected Success!")
        wait(math.random(1,3))
        Lib.Message.New("Fetching Pets ID As Server!")
        wait(math.random(1,3))
        Lib.Message.New("Starting To ReEquip!")

        for _, ID in next, PetIds do
            Lib.Network.Invoke("unequip pet", ID)
            Lib.Network.Invoke("equip pet", ID)
        end
        Lib.Message.New("ReEquip Success!\nSorry For The Invalid Response\nWe Will Work On The Fix\nPlutex Hub")
    end
    
    Lib.Message.New("Press Ok To Start ReEquip Function!")
    wait(math.random(1,3))
    ReEquip()
 end)
...
--- PLS I GOT OCD NOT axoegg do Axolotl_Egg
...
if GC then
    ...
    if getgenv().morespeed == true then
        for i,v in pairs(game:GetService("ReplicatedStorage").Game.Hoverboards:GetChildren()) do
            ...
            end local Lib = require(game.ReplicatedStorage:WaitForChild("Framework"):WaitForChild("Library"))
while not Lib.Loaded do
    game:GetService("RunService").Heartbeat:Wait();
end;
if game:GetService("CoreGui"):FindFirstChild("LuxtLibDexter Hub") then
        for i = 0,5 do
            game:GetService("CoreGui")["LuxtLibDexter Hub"]:Destroy()
            Lib.Message.New("Another menu is found. \n Please execute again.")
        end
end
